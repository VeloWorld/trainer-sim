...
- **D-VW-04:** VeloWorld installs trainer-sim via a **git-ref dependency** pinned to a specific commit:
  ...
  "trainer-sim": "github:agni21/trainer-sim#<sha>"
  ```
...
### Integration Points
- **VW's `package.json`** — gets one new `devDependencies` entry per D-VW-04. The git-ref form `github:agni21/trainer-sim#<sha>` is npm-resolvable on GitHub Actions macOS + Linux runners without extra setup.
